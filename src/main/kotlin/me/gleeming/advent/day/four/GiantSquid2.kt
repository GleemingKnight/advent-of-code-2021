...
class GiantSquid2 : AdventTask() {
    ...
    override fun run() {
        ...
        numbers.forEach { number ->
            val toRemove = ArrayList<HashMap<Int, List<Pair<Int, Boolean>>>>()
            boards.forEach { board ->
                ...
                if(canWin(board)) {
                    ...
                    if(boards.size == 1) {
                        ...
                        println("[Advent] A board has won (really lost) with a final score of ${sum * number}")
                        return
                    }
                }
            }
